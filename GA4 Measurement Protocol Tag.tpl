...
[
  ...
  },
  {
    ...
    "displayName": "Analytics Storage Consent",
    "macrosInSelect": true,
    "selectItems": [
      {
        "value": "GRANTED",
...
function consentValueToDigit(consentValue) {
    return consentValue === "GRANTED" ? "1" : "0";
}

// Verwerk consent-waarden
var adStorageConsentDigit = consentValueToDigit(data.ad_storage_consent);
var analyticsStorageConsentDigit = consentValueToDigit(data.analytics_storage_consent);

// Bouw de consentString voor de URL
var consentString = "G1" + adStorageConsentDigit + analyticsStorageConsentDigit;

// Voeg de consentString toe aan de URL
var url = "https://www.google-analytics.com/mp/collect?measurement_id=" + 
    data.measurementId + "&api_secret=" + data.apiSecret + "&gcs=" + consentString;
...
var payload = {
  ...
  consent: {
    ad_user_data: data.ad_user_data === "GRANTED" ? "GRANTED" : "DENIED",
    ad_personalization: data.ad_personalization === "GRANTED" ? "GRANTED" : "DENIED"
  }
};
